<section class="grid col-three-quarters mq2-col-full">
  ...
  <hr>
  <h4>＊為必填欄位</h4>
  <form action="<?php echo base_url (array ('admin', 'portfolios', 'edit', $portfolio->id));?>" method="post" enctype="multipart/form-data" >
    <article>
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tbody>
          <tr>
            <td width="80">分類＊</td>
            <td class="textleft">
              <select name='portfolio_tag_id'>
                <option value='0'>未分類</option>
          <?php if ($tags = PortfolioTag::all ()) {
                  foreach ($tags as $tag) { ?>
                    <option value='<?php echo $tag->id;?>'<?php echo $tag->id == $portfolio->portfolio_tag_id ? ' selected' : '';?>><?php echo $tag->name;?></option>
            <?php }
                } ?>
              </select>
            </td>
          ...
          <tr>
            <td>標題＊</td>
            <td class="textleft">
              <input type='text' name='title' value="<?php echo $portfolio->title;?>" placeholder='請輸入標題' maxlength='100' pattern=".{1,100}" required title="輸入100個字元以內" />
              &nbsp;&nbsp;輸入100個字元以內
            </td>
          </tr>
          <tr>
            <td>區域＊</td>
            <td class="textleft">
              <input type='text' name='address' value="<?php echo $portfolio->address;?>" placeholder='請輸入區域' maxlength='200' pattern=".{1,200}" required title="輸入200個字元以內" />
              &nbsp;&nbsp;輸入200個字元以內
            </td>
          </tr>
          <tr>
            <td>坪數＊</td>
            <td class="textleft">
              <input type='number' name='level' value="<?php echo $portfolio->level;?>" placeholder='輸入坪數(數字)' maxlength='100' pattern="\d*" required title="輸入坪數(數字)" />
              &nbsp;&nbsp;輸入坪數(數字)
            </td>
          </tr>
          <tr>
            ...
            <td class="textleft">
              ...
              <div class="pic">
                <ul>
            <?php foreach ($portfolio->pics as $pic) { ?>
                    <li>
          ...
          <tr>
            ...
            <td class="textleft">
              <select name='is_enabled'>
                <option value='1'<?php echo $portfolio->is_enabled ? ' selected': '';?>>上架</option>
                <option value='0'<?php echo $portfolio->is_enabled ? '' : ' selected';?>>下架</option>
              </select>
            </td>
          </tr>
        </tbody>
...
<?php foreach ($portfolio->blocks as $index => $block) {
        if ($block->type == 1) { ?>
